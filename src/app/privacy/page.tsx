...
const sections: { title: string; body: string[] }[] = [
  ...
  {
    title: "2. フォーム入力内容の送信先と保管",
    body: [
      "当サイトの相談フォームに入力された内容は、送信ボタンを押した時点で運営者へ送信され、運営者が管理する記録（表計算ファイル）および運営者のメールボックスに保管されます。送信の処理には Google LLC が提供する Google Apps Script および Google スプレッドシートを利用しています。",
      "これらは運営者のアカウント内で管理されるものであり、運営者以外の第三者が閲覧できる状態には置いていません。Google に対する情報の預託は、同社のプライバシーポリシーに従って行われます。",
      "また、LINEを通じてご相談いただいた場合は、その内容がLINEのトーク履歴として保管されます。",
      "保管した情報は、利用目的を達成し保有する必要がなくなった時点で消去します。消去をご希望の場合は、本ポリシー末尾の窓口までお申し出ください。",
    ],
  },
  {
    title: "3. 利用目的",
  ...
  },
  {
    ...
    body: [
      "当サイトでは、どのページがどれくらい閲覧されているかを把握するため、Cloudflare, Inc. が提供する Cloudflare Web Analytics を利用する場合があります。",
      "このツールはCookieを使用せず、閲覧者を個人として識別する情報も収集しません。取得されるのは、閲覧されたページのアドレス、参照元、ブラウザや端末の種別といった統計的な情報に限られます。",
      "収集された情報は、サイトの改善のためにのみ利用します。",
    ],
  },
  {
    title: "8. 本ポリシーの変更",
